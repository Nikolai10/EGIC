...
'''
import sys
sys.path.append("/tf/notebooks/yoloface")
...
'''
def get_yolo_output(image, crop_size=256):
    
    image = image.numpy()
    
    model = YoloDetector(target_size=crop_size, device="cuda:0", min_face=0)
    bboxes, _ = model.predict(image)
    
    return bboxes

def create_face_mask(yolo_output, image):
    
    binary_mask = np.zeros_like(image[:, :, 0])
    
    for image_boxes in yolo_output:
...
def get_yolo_output_ext(image, crop_size=256):
    
    model = YoloDetector(target_size=crop_size, device="cuda:0", min_face=0)
    bboxes, _ = model.predict(image)
    
    return create_face_mask(bboxes, image)
'''
...
class PanopticSampleGenerator:
  ...
  def call(self,
    ...
    '''
    temp = tf.py_function(func=get_yolo_output, inp=image, Tout=tf.float32)
    sample['face_mask'] = temp
    '''
